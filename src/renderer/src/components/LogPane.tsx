...
export default function LogPane(): JSX.Element {
  ...
  async function copyAll(): Promise<void> {
    const text = entries.map((e) => `${e.ts} [${e.level.toUpperCase()}] ${e.text}`).join('\n')
  ...
  return (
    <aside className="flex w-[420px] shrink-0 flex-col border-l border-slate-800 bg-black/60">
      <div className="flex items-center gap-2 border-b border-slate-800 px-3 py-2">
        ...
        <span className="min-w-0 flex-1 truncate text-[10px] text-slate-600" title={logPath}>
          {logPath}
        </span>
        <button
        ...
        <button
          onClick={() => void window.driverpick.revealLogFile()}
          className="rounded border border-slate-700 px-2 py-0.5 text-[10px] text-slate-400 hover:bg-slate-800"
          title="Show the log file in Explorer"
        >
          Open folder
        </button>
      </div>
